...
test('setCompilerOptions merges nested offload options', t => {
    const rt = new Runtime();
    rt.setCompilerOptions({
        offload: {
            enabled: true,
            minWasmIterations: 1024
        }
    });
    t.equal(rt.compilerOptions.offload.enabled, true);
    t.equal(rt.compilerOptions.offload.minWasmIterations, 1024);
    t.equal(rt.compilerOptions.offload.webgl, true);
    t.equal(rt.compilerOptions.offload.minWebGLElements, 4096);
    t.end();
});
